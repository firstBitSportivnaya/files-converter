...
- **`xml_files`**: *(массив)* - список файлов XML, в которые необходимо внести изменения. Например для изменения флага в общем модуле или добавление префикса конфигурации. Чтобы лучше понять настройку параметра xml_files, пользователи могут посмотреть файл с настройками `./configs/default.json`, где приведён пример использования. Этот файл показывает, как правильно структурировать параметры для XML-файлов и операций, чтобы эффективно управлять элементами XML в процессе конвертации.
